...
if [[ ! -f "$HF_MODEL_PATH/config.json" ]]; then
    ...
    echo "Run scripts/sft/ezpoints/train_sft.sh first, or set HF_MODEL_PATH explicitly." >&2
    exit 1
fi
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-seed_qwen2.5_vl_3b_ezpoints_sft_gemini_self_mean-std-norm}"
...
echo "Running EZPoints RL from Gemini-self SFT model"
...
exec bash "$SCRIPT_DIR/_common/ezpoints.sh" "${launch_args[@]}" "$@"
