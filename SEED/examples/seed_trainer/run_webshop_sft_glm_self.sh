...
if [[ -z "${HF_MODEL_PATH:-}" ]]; then
    if [[ -n "${MODEL_PATH:-}" ]]; then
        ...
        HF_MODEL_PATH="$MODELS_ROOT/Qwen2.5-3B-Instruct-webshop-episode-skill-sft-glm-self"
    else
...
if [[ ! -f "$HF_MODEL_PATH/config.json" ]]; then
    ...
    echo "Run scripts/sft/webshop/train_sft.sh first, or set HF_MODEL_PATH explicitly." >&2
    exit 1
fi
...
export PROJECT_NAME="${PROJECT_NAME:-agentic_webshop}"
...
echo "Running WebShop RL from GLM-self SFT model"
...
exec bash "$SCRIPT_DIR/_common/webshop.sh" "${launch_args[@]}" "$@"
